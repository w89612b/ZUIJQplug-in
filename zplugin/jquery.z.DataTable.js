...
* 2016-07-26 1、修改bug没有收索到数据分页的更新1801          
*/
(function($) {
	$.fn.extend({
        ZgetDataView: function(options) {
            if (!isValid(options)){alert("参数不合法！");return;}   //检测用户传进来的参数是否合法
			...
			Ttitle = '<div class="row">'+
            				...
            				'<div class="col-md-6"><div class="col-md-9 text-right"><p style="line-height:32px;margin:0">显示条数<p></div><div class="col-md-3 text-left">'+PageView(opts.page.pageList,opts.page.pageSize)+'</div></div>'+
            			'</div>';
			...
			$("#pageView").on('change',function () {
            	...
            	DbodyCreater(mydata,opts);
            	PpageListCreater(tabledata.total,viewSize,1,opts,tabledata);
			});
            //初始化
            this.init = function () {
            	...
            	$.ajax({
					...
					success :function (data) {
		            	DtheadCreater(data.thead,opts.checkbox,opts.operate);
						...
						if(data.total>0){
							...
							DbodyCreater(mydata,opts);
							PpageListCreater(data.total,viewSize,1,opts,data);
							tabledata = data;
						}else{
    ...
    //PageView构造方法
    function PageView (pageList,pageSize) {
    ...
    //头部构造方法
    function DtheadCreater(data,Select,operate) {
    	var $thead = ['<tr>'],temp='';
	 ...
	 */ 
    function PpageListCreater (total, pageSize, curPage,opts,tabledata,groups) {
    	var output='',
	    ...
	    $("#pageList li").on("click",function(){
			var pageNumber = parseInt($(this).children('a').attr('data-page'));
	    	var mydata = [],
	    		DR = tabledata.rows,
	    		viewSize=$("#pageView").val(),
	    	...
	    	//if(pots.async){
		    	...
		    	DbodyCreater(mydata,opts);
	            PpageListCreater(tabledata.total,viewSize,pageNumber,opts,tabledata);
            /*}else{
            	var param =  $(opts.queryParams).serializeArray();
            		param.push({"name":"module.pageSize","value":viewSize});
            		param.push({"name":"module.pageNumber","value":pageNumber});
        		$.ajax({
            		type:"POST",
	                async:false, 
	                url:opts.dataurl,
            		...
            		dataType : "json",
					success :function (data) {
							DbodyCreater(data.rows,opts);
							PpageListCreater(data.total,viewSize,pageNumber,opts,tabledata);	            	
            		}
				});
            }*/
